...
_This is part 2 of a 4-part series
...
`pixi run figs-post2` reproduces this page._
...
Where the complex number, $\omega = e^{2\pi i / N}$ — represents a single
...
$k$ times around the circle, sampled $N$ times — you can _see_ the frequencies rise
...
twice gives $W^2 = N \cdot P$, where $P$ reverses indices _circularly_:
...
eigenfunctions. The DFT isn't just _like_ a rotation\; it's very nearly a
quarter-turn.
...
below $f_s / 2$ is _exactly_ recoverable from its samples. Push past it and
...
rediscovered because it was _needed_\; the lesson about publishing your tricks writes
itself.
...
$O(N^2)$. But in the frequency domain it collapses to _pointwise multiplication_:
...
(One care: the DFT's convolution is _circular_ — index arithmetic wraps mod $N$,
...
Multiplication by a box in frequency _is_ convolution with a sinc in time:
...
fade the edges with a [_window_](https://en.wikipedia.org/wiki/Window_function)
...
knows _roughly when_, each row _roughly what frequency_, and the window length sets
...
- Trefethen, [_Spectral Methods in MATLAB_](https://people.maths.ox.ac.uk/trefethen/spectral.html) —
  chapter 1 is the spectral-derivative plot above, done first and better (programs and text free at that link).
- Brigham, _The Fast Fourier Transform and Its Applications_ — the butterfly, exhaustively.
