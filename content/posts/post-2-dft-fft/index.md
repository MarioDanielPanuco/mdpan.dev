...
series coefficients from part 1 become the **discrete Fourier transform**:
...
click of a clock hand with $N$ positions. Its powers are the **$N$-th roots of
unity**, and they are the DFT's entire alphabet:

![The 16th roots of unity arranged on the unit circle](01-roots-of-unity.svg)

Because the sum is finite, the DFT is literally a matrix-vector product
...
$\mathbb{C}^N$ — no information created or destroyed, just re-expressed. Parseval's
theorem ($\sum |x_n|^2 = \frac{1}{N}\sum |X_k|^2$) is nothing more than "rotations
preserve length."
...
The **Nyquist–Shannon theorem** draws the line: a signal containing frequencies only
...
Cooley and Tukey published in 1965, motivated in part by detecting Soviet nuclear
tests from seismometer data — a problem where transforms of long records had to be
feasible. Gauss had the same algorithm in an unpublished 1805 notebook, two years
before Fourier's memoir, for interpolating asteroid orbits. It was rediscovered
because it was *needed*\; the lesson about publishing your tricks writes itself.

## The convolution theorem: the workhorse

Here is the identity that makes the FFT an engine rather than a curiosity.
**Convolution** blends one sequence with another:
...
the full output length first. Same trick multiplies million-digit integers: digits
are polynomial coefficients, and carrying is a post-processing step.)
...
smears energy across every bin — **spectral leakage**. The fix is to fade the edges
with a *window* (Hann, Hamming, Blackman...) before transforming, trading a slightly
...
the **short-time Fourier transform** — the spectrogram, time-frequency currency of
speech, audio ML, and seismology. Here's one of the mp3s sitting in this repo's
`data/` folder:
...
everywhere. First, the DFT is the **z-transform** $X(z) = \sum x_n z^{-n}$ evaluated
at $N$ points on the unit circle — digital filter design is complex analysis on that
...
- Trefethen, *Spectral Methods in MATLAB* — chapter 1 is the spectral-derivative plot above, done first and better.
- Brigham, *The Fast Fourier Transform and Its Applications* — the butterfly, exhaustively.
- Heideman, Johnson & Burrus, "Gauss and the history of the FFT" (1984) — the 1805 notebook story.
