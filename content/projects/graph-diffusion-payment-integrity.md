...
operations, and review capacity is limited. This project is a
...
**decides** via the UI. Nothing is automatically blocked, approved, or reported.
...
exactly the shape you want from a _screening_ tool whose job is to widen the
...
Here's the subgraph this is computed on -- every vendor within two hops
...
A more advanced system would go along the path of graph optimization, particularly social network algorithmns.
When a payment is made, first check the graph diffusion score to locate suspicious transaction networks.
Then check to see if payments within the network are commom. For example, a local vendor will have transactions
with local customers, who also purchase goods and services from other local businesses. This will require us to
construct a the network/graph as a matrix, compute it's Laplacian graph matrix, solve the eigen problem on the Laplacian
then analyze the spectral embeddings.
[Stanford CS168: Spectral Graph Theory](https://web.stanford.edu/class/cs168/l/l11.pdf)
